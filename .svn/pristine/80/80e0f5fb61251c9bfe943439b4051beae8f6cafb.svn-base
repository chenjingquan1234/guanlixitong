<template>
  <div id="one">
    ...
    <mytable :tableTitle="tableTitle" :tableData="tableData" :operation_del_name="'启用'"
...
export default{
  ...
  methods:{
    ...
    del(row){
      this.$api.HttpPost("/StoreAdmin/Service/stopServiceItem", {id: row.row.id, status: 1})
      .then((data)=>{
    ...
    handlePageChange(data){
      ...
      this.$refs.mytable.findList(this.$api.yuming + "/StoreAdmin/Service/getStopServiceItemList", data);
    },
    unit_change(s){
      this.add_num.unit = s;
    },
