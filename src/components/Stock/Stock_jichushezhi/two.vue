<template>
  <div id="two">
    ...
    <el-dialog title="商品资料" :visible.sync="dialog" :close-on-click-modal="false">
      <el-row :gutter="10">
        <el-col :span="12">
          ...
          <el-input size="mini" v-if="showKucun" v-model="d.x"><label slot="prepend">现有库存</label></el-input>
        </el-col>
        <el-col :span="12">
    ...
    <el-dialog title="商品分类" :visible.sync="dialog1" :close-on-click-modal="false">
      ...
      <el-select size="mini" style="width: 100%;" v-model="dialog1_b" placeholder="所属分类" clearable @clear="pid_clear">
...
export default{
  ...
  data(){
    return{
      ...
      key_name: "",
      state_name: 1,
      state_select: [{label: "启用", id: 1}, {label: "停用", id: 0}],
      type_name: "",
      type_id: "",
      type_select: [],
  ...
  methods: {
    ...
    download(){
      ...
      window.location.href = this.$api.yuming + `/storeadmin/inventory/getProductData?sid=${this.store_name}&status=${this.state_name}&classify_id=${this.type_name}&search=${this.key_name}`;
