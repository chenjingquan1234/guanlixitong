<template>
  <div id="mytable">
    <!-- 表格 -->
    <vxe-table size="mini" align="center" :height="height" ref="xTable" highlight-hover-row highlight-current-row border
    ...
    @edit-closed="edit_closed">
      ...
      :field="value.key" :title="value.title" :width="value.width" :min-width="value.min_width" :edit-render="value.edit" :tree-node="value.tree">
      </vxe-table-column>

      <vxe-table-column v-if="operation" title="操作" min-width="150">
        <template slot-scope="scope">
          <el-button v-if="operation_mod" size="mini" type="text" @click="modify(scope, 2)">{{operation_mod_name}}</el-button>
          <el-button v-if="operation_del" style="color: red;" size="mini" type="text" @click="del(scope)">{{operation_del_name}}</el-button>
          <el-button v-if="operation_" class="operation" size="mini" type="text" @click="operat(scope)">{{operation_name}}</el-button>
        </template>
      </vxe-table-column>
    </vxe-table>
    <!-- 分页器 -->
    <vxe-pager v-if="paging"
      size="mini"
      perfect
      :loading="loading"
      :pager-count="5"
